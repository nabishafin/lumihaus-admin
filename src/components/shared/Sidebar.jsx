...
export default function Sidebar() {
  ...
  return <aside className="sidebar"><div className="brand"><span className="brand-mark">L</span><div><strong>LumiHaus</strong><small>Beauty commerce</small></div></div><button className="collapse-button" onClick={()=>setCollapsed(!collapsed)} aria-label="Toggle sidebar"><ChevronLeft size={15}/></button><nav>{links.map(([to,label,Icon]) => <NavLink title={label} key={to} to={to} end={to === "/"} className={({isActive}) => isActive ? "nav-link active" : "nav-link"}><Icon size={18}/><span>{label}</span></NavLink>)}</nav><div className="sidebar-user"><span className="avatar">SA</span><div><strong>Shafin Ahmed</strong><small>Super Admin</small></div><LogOut size={16}/></div></aside>;
}
